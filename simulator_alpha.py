...
class Ui_Panel(object):
    ...
    def preCompute(self):
        ...
        if (flag1 == 1) & (flag2 == 1):
            ...
            #p1 = Ui_Panel.toMatrix(self.input_p1.text())
            # = Ui_Panel.toMatrix(self.input_p2.text())
            p1 = [-4.2563, -27.4104]
            p2 = [-4.2563, -27.4104]
            ...
            for t in np.arange(t_att, tm-0.01, 0.01):
                ...
                fxe0[:, int(100 * t + 1)] = np.transpose(mat(mat(T)*mat(np.row_stack((mat(xl),mat(mat(U1)*mat(y)-mat(U1)*mat(C)*mat(N)*mat(xl)))))))
                #if(t==0.5):
                    # = [[-3.293564538923253],[4.999227379354861]]
                #if(t>=0.5) & (t<=0.55):
                    #print(int(100*t+1))
                    #print(fq0[:, int(100 * t + 1)])
                    #print(xl)
                    #print(fxe0[:,int(100*t+1)])
    ...
    def preSet(self):
        ...
        self.input_p1.setText('-4.2563,-27.4104')
        self.input_p2.setText('-4.2563,-27.4104')
